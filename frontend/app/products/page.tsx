...
function ProductsContent() {
  ...
  const fetchProducts = async () => {
    setLoading(true)
    try {
      const params: any = { sort: filters.sort, page, limit: 20 }
        ...
        {showFilters && (
          <div className="bg-white rounded-2xl shadow-sm p-4 mb-4 flex flex-wrap gap-3 items-end">
            ...
            <button onClick={clearFilters}
              className="flex items-center gap-1 px-3 py-2 text-sm text-red-500 border-2 border-red-100 rounded-xl hover:bg-red-50 transition-colors">
              <X size={14} /> Clear
            </button>
          </div>
        )}
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-0 border border-gray-200 bg-white rounded-sm">
            {Array(10).fill(0).map((_, i) => (
              <div key={i} className="p-3 border border-gray-100">
                <div className="aspect-square bg-gray-100 rounded-lg mb-3 animate-pulse" />
...
export default function ProductsPage() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-[#f1f3f6] flex items-center justify-center"><div className="animate-spin w-8 h-8 border-4 border-orange-500 border-t-transparent rounded-full"/></div>}>
      <ProductsContent />
    </Suspense>
  )
}